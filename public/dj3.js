...
clearAllBtn?.addEventListener("click", async () => {
  ...
  try {
    await fetch("/api/requests3", { method: "DELETE" });
  } catch (e) {
...
// ✅ estado pedidos (piso3) -> FRANJA inferior verde/roja
socket.on("orders:status", (st) => {
  const isOpen = !!st?.piso3;
...
function render(requests) {
  ...
  // ✅ nombre (cliente) por mesa: el primer pedido que aparece para esa mesa
          ...
          .map((r, idx) => {
            ...
            return `
              <div class="song-item ${isLastNew ? "new-song" : ""}">
                ...
                <!-- ✅ NOMBRE ARRIBA (SIN 'Cliente:' / 'Pedido por:') -->
                <div class="song-client">${escapeHtml(r.name)}</div>
...
cards.addEventListener("click", async (e) => {
  ...
  if (pendingConfirmId === id) {
    ...
    try {
      const res = await fetch(`/api/requests3/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || `Error ${res.status}`);
      }

      const listRes = await fetch("/api/requests3");
  ...
  // 1er click = dejar en modo confirm
  pendingConfirmId = id;
...
// ✅ socket piso3
socket.on("requests3:update", (requests) => render(requests));
